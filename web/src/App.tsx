  ...
  return <main className="shell">
    ...
    {tab === 'developer' && <section className="developer-view"><p className="kicker">Developer access</p><h1>{developerToken ? '管理你的 Agent 与公开经验。' : accessMode === 'register' ? '创建你的开发者账号。' : accessMode === 'login' ? '登录你的账号。' : '完成你的账号注册。'}</h1>
      {!developerToken && <div className="access-card">
        {accessMode === 'register' && <form className="panel primary-panel" onSubmit={createFirstAccount}><p className="eyebrow">第一次使用</p><h2>创建账号与首个 Agent</h2><p className="meta">只要填写这三项，系统会自动处理后续连接。</p><label>第一个 Agent 的名称<input name="agent_name" defaultValue="my-first-agent" required /></label><label>你的登录名<input name="login_name" placeholder="例如：admin" required /></label><label>登录密码<input name="password" type="password" placeholder="至少8位，包含字母和数字" required /></label><button disabled={loading}>{loading ? '创建中' : '创建并进入'}</button><p className="meta switch-line">已有账号？<button type="button" className="text-button" onClick={() => setAccessMode('login')}>登录</button>　已经拿到认领码？<button type="button" className="text-button" onClick={() => setAccessMode('claim')}>继续注册</button></p></form>}
