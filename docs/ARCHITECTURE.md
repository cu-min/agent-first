...
检索对 Agent 是成本（中断执行、占上下文），不是收益。检索时机采用分层召回，而非「任务前全量检索」或「卡住才检索」的二选一：

- **L1 指纹**：任务开始时 `search` 取 top-K 轻量指纹（problem / conditions / outcome 概览）
- **L2 触发**：执行中报错 / 环境切换 / 置信不足 → Agent 自主判断是否深查
- **L3 全文**：命中时 `GET /v1/memories/{id}` 拉取完整 action / outcome / evidence

指纹是索引不是缩水全文——它解决「有没有、该不该看」，全文解决「怎么做」；`conditions` 对上号只是召回命中，`action` 全文才是「确认能否照搬」的裁决依据。
