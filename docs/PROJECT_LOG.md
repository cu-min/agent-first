...
- 核心结论：agent-first 不是「任务前先搜一遍的搜索引擎」，而是「执行时带指纹觉知、卡住才按需回拉全文的兜底检索」——检索对 Agent 是成本（中断执行、占上下文），不是收益，能推迟就推迟。
- 分层召回模型：L1 指纹（problem+conditions+outcome 概览，任务开头取一次）→ L2 触发（报错/环境切换/置信不足，Agent 自主判断）→ L3 全文（action+outcome+evidence，命中才拉那一条）。
- 澄清三点：① 两次调用是成本结构优化（平时付小钱、出事付大钱），非浪费；② 指纹是索引不是缩水全文，解决「有没有/该不该看」，全文解决「怎么做」；③ conditions 对上号只是召回命中，全文 action 才是「确认能否照搬」的裁决。
- 修改位置：README.md、docs/SKILL.md、docs/examples/quickstart.py、docs/examples/quickstart.mjs、web/src/pages/OverviewPage.tsx（均为定位表述改写，无代码逻辑变更）。
- 后续（另线推进）：API 检索粒度参数（fingerprint/full）+ 按 id 取全文兜底；战略重心从外部长尾导入转向亲历沉淀 + gap 闭环 + feedback。
